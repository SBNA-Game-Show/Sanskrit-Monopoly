...
// Explicitly import your images so Vite bundles them properly
...
export default function Lobby() {
  ...
  // Tracks which token is assigned to which player slot
  const [playerTokens, setPlayerTokens] = useState<(string | null)[]>([null, null, null, null]);

  const hostPlayer = { name: username || "Bob", role: "host" };
  ...
  const canStart = isHost && selectedEdition !== null && startingMoney !== null;
  ...
  const handleDropOnPlayer = (e: React.DragEvent, playerIndex: number) => {
    ...
    setPlayerTokens(prev => {
      ...
      if (existingIndex !== -1) newTokens[existingIndex] = null;
      newTokens[playerIndex] = tokenId;
      return newTokens;
    });
  };
  ...
  return (
    <main className="h-[calc(100vh-56px)] overflow-hidden bg-white font-jersey flex flex-col justify-between select-none">
      ...
      {/* Inline style for the pulsing neon glow effect on the Start Button */}
      <style>
        {`
      ...
      <div className="w-full max-w-6xl mx-auto px-4 py-2 lg:py-4 grid grid-cols-1 lg:grid-cols-5 gap-4 lg:gap-6 items-center flex-grow min-h-0">
        ...
        {/* LEFT SECTOR: PLAYER REGISTRATION MATRIX */}
        <div className="lg:col-span-3 flex flex-col items-center justify-center space-y-4 w-full h-full">
          ...
          <div className="w-full flex flex-col space-y-3 max-w-xl">
            ...
            <div className="flex justify-center w-full">
              <div className="w-full sm:w-[calc(50%-0.5rem)] bg-[#FFC17E] p-2 rounded-2xl flex justify-between items-center shadow-[0px_4px_4px_0px_rgba(0,0,0,0.5)] border border-white/20">
                <span className="text-2xl lg:text-3xl tracking-wider text-white pl-4 drop-shadow-[0_2px_2px_rgba(0,0,0,0.3)]">
                  {hostPlayer.name}
                </span>
                <div className="w-12 h-12 lg:w-14 lg:h-14 bg-white rounded-2xl flex items-center justify-center p-1 shadow-inner overflow-hidden mr-1 border border-orange-200">
                  <img src={hostImg} alt="Host" className="w-full h-full object-contain" draggable="false" />
                </div>
              </div>
            </div>

            {/* Generic Players Grid with Drop Zones */}
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 w-full">
              {genericPlayers.map((p, index) => (
                <div key={p.id} className="bg-[#FFC17E] p-2 rounded-2xl flex justify-between items-center shadow-[0px_4px_4px_0px_rgba(0,0,0,0.5)] border border-white/20">
                  <span className="text-2xl lg:text-3xl tracking-wider text-white pl-4 drop-shadow-[0_2px_2px_rgba(0,0,0,0.3)]">
                    {p.name}
                  </span>
                  
                  {/* DROP ZONE FOR THIS PLAYER */}
                  <div 
                    onDragOver={(e) => e.preventDefault()}
                    onDrop={(e) => handleDropOnPlayer(e, index)}
                    className="w-12 h-12 lg:w-14 lg:h-14 bg-white rounded-2xl flex items-center justify-center shadow-inner mr-1 transition-colors hover:bg-orange-50"
                  >
                    {playerTokens[index] ? (
                      <div 
                        draggable
                        onDragStart={(e) => handleDragStart(e, playerTokens[index]!)}
                      ...
                      >
                        <img 
                          src={availableTokens.find(t => t.id === playerTokens[index])?.src} 
                          alt="token"
                          className="h-8 w-8 lg:h-10 lg:w-10 object-contain pointer-events-none"
                        />
                      </div>
                    ) : (
          ...
          >
            {availableTokens.map((token) => {
              ...
              return (
                <div key={token.id} className="w-16 h-16 lg:w-20 lg:h-20 flex items-center justify-center">
                  {!isPlaced ? (
                    <div 
                      draggable
                    ...
                    >
                      <img 
                        src={token.src}
                        alt={token.id}
                        className="h-10 w-10 lg:h-12 lg:w-12 object-contain pointer-events-none"
                      />
                    </div>
                  ) : (
                    // Faded placeholder where the token used to be
        ...
        {/* RIGHT SECTOR: DYNAMIC INTERACTIVE OPTIONS BOARD */}
        <div className="lg:col-span-2 bg-[#FFC17E] p-6 lg:p-8 rounded-3xl flex flex-col shadow-[0px_4px_4px_0px_rgba(0,0,0,0.5)] w-full h-full max-h-none lg:max-h-[460px]">
          
          <h2 className="text-3xl lg:text-4xl text-center mb-8 lg:mb-10 tracking-widest text-white drop-shadow-[0_4px_4px_rgba(0,0,0,0.5)]">
            GAME SETTINGS
          </h2>
          ...
          <div className="flex flex-col flex-grow space-y-10">
            {/* EDITION SELECTOR */}
            <div>
              <span className="block text-lg lg:text-xl text-white mb-3 uppercase tracking-wider">Edition</span>
            ...
            {/* CURRENCY SELECTOR */}
            <div>
              <span className="block text-lg lg:text-xl text-white mb-3 uppercase tracking-wider">Starting Money</span>
      ...
      {/* FOOTER PURE TAILWIND START BUTTON */}
      <div className="w-full bg-[#FFC17E] border-t-4 border-[#FF8C00] flex justify-center items-center h-20 lg:h-24 shrink-0 shadow-[0px_-4px_10px_rgba(0,0,0,0.15)] z-20">
        <button
          disabled={!canStart}
          onClick={handleStartGame}
          className={`
            w-[220px] h-[50px] lg:w-[240px] lg:h-[55px] rounded-2xl text-2xl lg:text-3xl font-jersey tracking-widest text-white transition-all duration-300 relative bottom-1 border-none
            ${canStart 
